...
public class CartridgeAssemblyRecipe extends CustomRecipe {
		...
		public CartridgeAssemblyRecipe(ResourceLocation location) { super(location, CraftingBookCategory.MISC); }
		...
		public boolean matches(CraftingContainer container, Level level) {
			...
			for (int i = 0; i < container.getContainerSize(); ++i) {
				...
				if (stack.getItem() instanceof MediumcannonRoundItem) {
					...
					roundPosition = i;
					if (stack.getItem() instanceof FuzedMediumItemMunition) searchFuze = true;
				} else if (stack.getItem() == CBCModernWarfareItem.FILLED_MEDIUMCANNON_CARTRIDGE.get()) {
		...
		public ItemStack assemble(CraftingContainer container, RegistryAccess access) {
			...
			if (container.getItem(roundPosition).getItem() instanceof MediumcannonRoundItem round) {
				result = round.getCartridgeType();
			}
		...
		@Override public RecipeSerializer<?> getSerializer() { return CBCModernWarfareRecipeTypes.MEDIUM_CARTRIDGE_ASSEMBLY.getSerializer(); }
}
