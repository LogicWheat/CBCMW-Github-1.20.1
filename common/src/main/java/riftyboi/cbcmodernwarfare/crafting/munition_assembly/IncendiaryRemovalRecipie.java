...
public class IncendiaryRemovalRecipie extends CustomRecipe {


	public IncendiaryRemovalRecipie(ResourceLocation id) {
	...
	public ItemStack assemble(CraftingContainer container, RegistryAccess registryAccess) {
		return new ItemStack(CBCModernWarfareItem.INCENDIARY_TIP.get());
	}
		...
		for (int i = 0; i < sz; ++i) {
			...
			if (stack.getItem() instanceof AutocannonRoundItem) {
				if (stack.getOrCreateTag().getBoolean("Incendiary")) {
					...
					copy.setCount(1);
					copy.getOrCreateTag().remove("Incendiary");
					if (isCartridge) {
				...
				if (mixinItem.isIncendiary(stack)) {
					ItemStack copy = stack.copy();
					copy.setCount(1);
					mixinItem.setIncendiary(copy, false);
					if (isCartridge) {
						ItemStack cartridge = new ItemStack(originalStack.getItem());
						AutocannonCartridgeItem.writeProjectile(copy, cartridge);
						result.set(i, cartridge);
	...
	@Override public RecipeSerializer<?> getSerializer() { return CBCModernWarfareRecipeTypes.INCENDIARY_REMOVAL.getSerializer(); }
